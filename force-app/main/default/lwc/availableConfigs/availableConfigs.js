...
const INFO_NOCONFIG_MESSAGE =
  "All selections are already added in case configs";
...
export default class AvailableConfigs extends LightningElement {
  ...
  get isNextDisabled() {
    return (
      (this.configs && this.configs.length < this.limit) || this.hasNoMorePages
    );
  }
  gotoNext() {
  ...
  gotoPage(event) {
    this.navigateToPage(parseInt(event.target.dataset.id, 10));
  }
  gotoPrevious() {
    this.navigateToPage(this.currentPage - 1);
  }
  navigateToPage(pageNo) {
    if (this.currentPage !== 0) {
      const oldSelectedButton = this.template.querySelector(
    ...
    const currentButton = this.template.querySelector(
      "[data-id='" + pageNo + "']"
    );
    if (currentButton) {
